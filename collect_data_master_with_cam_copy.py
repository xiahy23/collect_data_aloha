...
def save_data(args, timesteps, actions, dataset_path):
    ...
    with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024 ** 2 * 2) as root:
        ...
        for cam_name in args.camera_names:
            _ = image_grp.create_dataset(
                cam_name, (data_size, 480, 640, 3), dtype='uint8',
                chunks=(1, 480, 640, 3))
...
class RosOperator:
    def __init__(self, args):
        ...
        # 订阅相机
        rospy.Subscriber(args.img_front_topic, Image,
                         self._img_front_cb, queue_size=1000, tcp_nodelay=True)
        rospy.Subscriber(args.img_left_topic, Image,
                         self._img_left_cb, queue_size=1000, tcp_nodelay=True)
        rospy.Subscriber(args.img_right_topic, Image,
                         self._img_right_cb, queue_size=1000, tcp_nodelay=True)
    ...
    def get_synced_frame(self):
        """时间同步：取所有 deque 的最新公共时间戳，拉齐各传感器数据"""
        # 检查所有 deque 非空
        if (len(self.img_front_deque) == 0 or len(self.img_left_deque) == 0 or
                len(self.img_right_deque) == 0 or
                len(self.master_arm_left_deque) == 0 or
                len(self.master_arm_right_deque) == 0):
            return None

        # 取各 deque 最新帧的时间戳中的最小值作为同步基准
        frame_time = min(
        ...
        # 确认每个 deque 的最新帧 >= frame_time（否则某传感器还没跟上）
        ...
        if self.img_right_deque[-1].header.stamp.to_sec() < frame_time:
            return None
        if self.master_arm_left_deque[-1].header.stamp.to_sec() < frame_time:
            return None
        if self.master_arm_right_deque[-1].header.stamp.to_sec() < frame_time:
            return None

        # 跳过早于 frame_time 的旧帧，取第一个 >= frame_time 的帧
    ...
    def process(self):
        ...
        while not rospy.is_shutdown():
            if (len(self.img_front_deque) > 0 and len(self.img_left_deque) > 0 and
                    len(self.img_right_deque) > 0 and
                    len(self.master_arm_left_deque) > 0 and
                    len(self.master_arm_right_deque) > 0):
                print("\033[32m所有数据源就绪!\033[0m")
                break
        # CAN 数据稳定等待：等第一包真实关节数据（非零）到来，最多等 5s
        print("等待主臂 CAN 数据稳定...")
        t_can = time.time()
        while not rospy.is_shutdown():
            def _arm_valid(dq):
        ...
        while count < self.args.max_timesteps + 1 and not rospy.is_shutdown():
            ...
            action = qpos.copy()  # 无从臂时，action = 主臂 qpos
...
def get_arguments():
    ...
    # 主臂话题
    parser.add_argument('--master_arm_left_topic', type=str,
                        default='/master/joint_left')
    parser.add_argument('--master_arm_right_topic', type=str,
