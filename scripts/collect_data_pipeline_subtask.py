...
Extended with per-episode subtask sequencing.
...
# ------------------------------------------------------------------
# Subtask list — edit here to define the steps for each episode.
# The pedal advances through these in order during a recording.
# ------------------------------------------------------------------
# SUBTASKS = [
#     "reach the handle of the lid",
#     "Grasp the handle of the lid",
#     "Move away the lid",
#     "reach the eggplant",
#     "grasp the eggplant",
#     "Move the eggplant on the box",
#     "Release the eggplant to the box",
...
#     "reach the cuboid",
#     "grasp the cuboid",
#     "Move the cuboid on the hole",
#     "Place the cuboid in the hole",
# ]
...
# SUBTASKS = [
#    "reach for food on the table",
#    "grasp the food",
#    "move food to the plate",
#    "release the food",
#    "reach for food on the table",
#    "grasp the food",
#    "move food to the plate",
...
SUBTASKS_1 = [
    ...
    "Reach plate",
    "Hold plate",
    "Move plate into open drawer",  # 保持hold状态直接移入抽屉
    "Release plate",
    "Close drawer",
]
...
def save_episode(args, timesteps, actions, subtask_sequence, dataset_path, instruction_text):
...
class CollectorWorker(threading.Thread):
    ...
    def request_start(self, instruction, folder, idx, subtasks=None):
        with self._lock:
    ...
    def _finalize_episode(self):
        ...
        try:
            ...
            save_episode(self.args, timesteps, actions, subtasks, dataset_path, meta["instruction"])
...
class CollectorApp:
    def __init__(self, args):
        ...
        self.root = tk.Tk()
        self.root.title("Data Collection Pipeline — Subtask Mode")
    ...
    def _build_ui(self):
        ...
        # Show subtask[0] if subtasks were provided on the CLI
        subtasks = getattr(self.args, "subtasks", None) or []
        if subtasks:
    ...
    def _on_start(self):
        ...
        if not ins:
            ...
            return
        subtasks = list(getattr(self.args, "subtasks", None) or [""])
        folder = self.meta.folder_for(ins)
        idx = next_episode_idx(folder)
        if self.worker.request_start(ins, folder, idx, subtasks):
            self._update_subtask_display(0, len(subtasks), subtasks[0])
            try:
        ...
        try:
            while True:
                ...
                if kind == "status":
                    ...
                    self._refresh_instructions()
                    # Reset subtask display to subtask[0] for the next episode
                    subtasks = list(getattr(self.args, "subtasks", None) or [""])
                    self._update_subtask_display(0, len(subtasks), subtasks[0])
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        help=(
            "Ordered list of subtask strings for each episode. "
            "Each pedal press during recording advances to the next subtask; "
            "the final press (after the last subtask) stops and saves. "
            "Example: --subtasks \"reach handle\" \"grasp\" \"lift\""
        ),
    )
